...
const signToken = (id) => {
  const token = jwt.sign({ id }, process.env.JWT_SECRET, {
    expiresIn: process.env.JWT_EXPIRES_IN, // Assumes this is a numeric string (in seconds or days)
  });
  return token;
};
...
const createSendToken = (user, statusCode, res) => {
  ...
  // Ensure JWT_EXPIRES_IN is a numeric value representing days
  const expiresInDays = Number(process.env.JWT_EXPIRES_IN) || 90; // Default to 90 if not provided

  const cookieOptions = {
    expires: new Date(Date.now() + expiresInDays * 24 * 60 * 60 * 1000), // Expires in days
    httpOnly: true, // Prevent client-side access to cookie
  };

  if (process.env.NODE_ENV === "production") cookieOptions.secure = true; // Ensure secure cookie in production
